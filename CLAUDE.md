...
- **If a change touches `web/static/js`**: `go test`/`curl` smoke-testing doesn't execute client-side JS. Verify with an actual browser once the dashboard exists (see servicedesk's `CLAUDE.md` for the Playwright-based pattern to copy).
