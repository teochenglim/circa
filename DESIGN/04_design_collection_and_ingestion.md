...
§4.1 rejected importing `node_exporter/collector` because the benefit (avoiding one extra scrape hop) was marginal once scraping localhost is nearly free. v0.5.0 revisits that tradeoff and reaches the opposite conclusion, for a different reason than the one §4.1 rejected: **onboarding**, not scrape-hop cost. A fresh install with an empty scrape target list showed nothing until a user manually stood up and pointed at a node_exporter — dead weight for [DESIGN/09](09_design_tech_stack_and_roadmap.md) §9.3's "zero-config onboarding" open question, and a blocker for v0.6.0's Netdata-style dashboard, which needs real data to render without any setup at all.
