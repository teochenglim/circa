...
web/                           — [BUILT: v0.6.0, +Self-metrics page v1.0.0] 9 pages (Overall + one per collector category + Metrics + Self-metrics), static HTML/CSS/JS, uPlot, embedded via go:embed
```
...
| `circa config init`/`check`, `circa auth add-user`/`reset-password`/`hash-password`, `circa backup-agent`, `circa sizing` | **Built: v0.3.0 (config/auth), v0.7.0 (backup-agent), v1.0.0 (sizing).** `cmd/circa/config_cli.go`, `cmd/circa/auth_cli.go`, `cmd/circa/backup_cli.go`, `cmd/circa/sizing_cli.go` — see [DESIGN/08](DESIGN/08_design_config_auth_ops.md) §8.1.2, [DESIGN/07](DESIGN/07_design_backup.md) §7.3, and [DESIGN/03](DESIGN/03_design_storage.md) §3.3 |
| HTTP handlers + routes | **Built: `query_range`, `series`, `alerts`, `anomalies`, `selfmetrics`, `metrics`, `status`, `healthz`, `readyz`, dashboard (`/`, `/static/*`), write receiver (`POST push.receive.path`, when `features.push_receive` is on), `backup_range` (`GET /api/v1/backup_range`, when `features.backup` and `backup.mode: pull`).** `internal/httpapi/` — `/healthz`/`/readyz` stay unauthenticated even when `auth.users` is set, everything else goes through `internal/auth.Middleware`. Every request is also recorded into circa's own RED self-metrics (`instrumentHTTP`, see "Self-metrics" below). `GET /api/v1/selfmetrics` (`selfmetrics_api.go`, v1.0.0) is a JSON snapshot of the same `prometheus.DefaultGatherer` state `GET /metrics` encodes as text — polled by the dashboard's Self-metrics page (`web/static/js/selfmetrics.js`), which builds its own short client-side rolling history rather than reading one back out of `internal/storage`, since self-metrics were never written there |
